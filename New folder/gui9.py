...
# Original reference dimensions (from your CSV files)
ORIGINAL_WIDTH = 1023
ORIGINAL_HEIGHT = 600

# Get screen dimensions
win = Tk()
win.withdraw()  # Hide window temporarily to get screen info
SCREEN_WIDTH = win.winfo_screenwidth()
SCREEN_HEIGHT = win.winfo_screenheight()
win.deiconify()  # Show window again
...
X_OFFSET = (SCREEN_WIDTH - (ORIGINAL_WIDTH * SCALE_FACTOR)) / 2
Y_OFFSET = (SCREEN_HEIGHT - (ORIGINAL_HEIGHT * SCALE_FACTOR)) / 2
...
for r in images:
    ...
    x, y = scale_position(r["x_pos"], r["y_pos"])
    img_ids[r["variable_name"]] = cv.create_image(x, y, image=img)
if "interrupt_light" in img_ids:
    cv.itemconfigure(img_ids["interrupt_light"], state="hidden")

txt_ids = {}
for t in texts:
    x, y = scale_position(t["x_pos"], t["y_pos"])
    font_size = scale_font_size(abs(int(t["font_size"]))) * (-1 if int(t["font_size"]) < 0 else 1)
    txt_ids[t["key"]] = cv.create_text(x, y, anchor="nw", text=t["text"], 
                                      fill=t["color"], font=(t["font_name"], font_size))

# ===== Camera =====
# Scale camera parameters dynamically
PREVIEW_W, PREVIEW_H = int(410 * SCALE_FACTOR), int(240 * SCALE_FACTOR)
PREVIEW_X, PREVIEW_Y = scale_position(262.0, 298.0)
RADIUS = int(25 * SCALE_FACTOR)

cap, camera_ready = None, False
left_video_id = cv.create_image(PREVIEW_X, PREVIEW_Y, image=None)
right_id, latest_frame, captured_path, prev_flag, last_mtime = None, None, None, None, None
...
def init_cam():
    ...
    for a in range(3):
        try:
            ...
            if cap.isOpened(): 
                camera_ready = True
                logging.info(f"Camera OK on try {a+1}")
                return
            cap.release()
        except Exception as e:
            logging.error(f"Cam init fail {a+1}: {e}")
        time.sleep(1)
...
def send_data_to_serial(data):
    """Write the command to serial input file."""
    try:
...
def monitor_stable_log():
    ...
    try:
        ...
        logging.error(f"Stable log monitor: {e}")
        ...
        if camera_ready and DATA_FILE.exists():
            ...
            if prev_flag == 0 and flag == 1:
                ...
                if capture_frame():
                    ...
                    img_copy.putalpha(mask)
                    imgtk = ImageTk.PhotoImage(image=img_copy)
                    # Scale the right camera position
                    right_x, right_y = scale_position(754.0, 297.0)
                    right_id = cv.create_image(right_x, right_y, image=imgtk)
