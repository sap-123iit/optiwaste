...
ImageDraw.Draw(left_mask).rounded_rectangle((0, 0, LEFT_PANE_W, LEFT_PANE_H), 50, fill=255)
ImageDraw.Draw(right_mask).rounded_rectangle((0, 0, RIGHT_PANE_W, RIGHT_PANE_H), 50, fill=255)
...
def init_cam():
    ...
    try:
        cap = cv2.VideoCapture(0)  # Use camera source 0
        if not cap.isOpened():
            logging.error("Failed to open camera source 0")
            camera_ready = False
            return
            
        # Set camera resolution to match left pane size
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, LEFT_PANE_W)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, LEFT_PANE_H)
        cap.set(cv2.CAP_PROP_FPS, 30)
        
        camera_ready = True
        logging.info("OpenCV camera initialized (source 0)")
            ...
            if last_mtime is None:
                ...
                if w and captured_path and captured_path.exists():
                    ...
                    if right_id:
                        cv.delete(right_id)
                        right_id = None
                        # Show the right camera pane background again
            ...
            if prev_flag == 0 and flag == 1:
                ...
                def delayed_capture():
                    ...
                    if capture_frame():
                        ...
                        # Resize and apply right pane mask
                        img_copy = latest_frame.copy()
                        ...
                        # Place captured image exactly over right camera pane
                        if right_pane_info:
                            right_id = cv.create_image(right_pane_info["x_pos"], right_pane_info["y_pos"], image=imgtk)
                        else:
                            right_id = cv.create_image(1427.85693359375, 562, image=imgtk)  # Fallback
                            
                        # Hide the right camera pane background
